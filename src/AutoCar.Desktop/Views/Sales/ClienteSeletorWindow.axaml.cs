...
/// via code-behind (padrão do projeto). Reusa IClienteService — não duplica busca. Plano B do
/// AutoCompleteBox (que renderizava quebrado no Avalonia 11).
/// </summary>
public partial class ClienteSeletorWindow : Window
{
    // CÓDIGO · NOME / RAZÃO SOCIAL · DOCUMENTO · TELEFONE
    private const string ColDefs = "70,*,150,130";
    private static readonly string[] Headers = { "CÓDIGO", "NOME / RAZÃO SOCIAL", "DOCUMENTO", "TELEFONE" };
    ...
    private readonly List<ClienteListaDto> _resultado = new();
    ...
    private ScrollViewer CriarCorpo()
    {
        ...
        for (var i = 0; i < _resultado.Count; i++)
        {
            ...
            Celula(grid, i, 0, c.CodCliente.ToString("0000"), mono: true);
            Celula(grid, i, 1, c.RazaoSocial);
            Celula(grid, i, 2, FormatarDocumento(c.Documento), mono: true);
            Celula(grid, i, 3, c.Telefone ?? "");
        }
    ...
    private static Border CriarMensagemVazia() => new()
    {
        ...
        Child = new TextBlock
        {
            Text = "Nenhum cliente encontrado. Ajuste a busca ou use \"Consumidor (sem cliente)\".",
            FontSize = 12,
    ...
    // TryParse tolerante: se o documento não for puramente numérico (dado legado/mal-formado),
    // exibe o valor cru em vez de lançar exceção e derrubar a renderização da tabela.

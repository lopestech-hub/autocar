...
/// <summary>
/// Listagem de mecânicos. Tabela renderizada como Grid único via code-behind (padrão do projeto —
/// nunca DataGrid). Duplo-clique numa linha abre o formulário.
/// </summary>
public partial class MecanicosView : UserControl
{
    // CÓDIGO · NOME · TELEFONE · STATUS
    private const string ColDefs = "80,*,140,90";
    private static readonly string[] Headers = { "CÓDIGO", "NOME", "TELEFONE", "STATUS" };
    ...
    private void AoTrocarDataContext(object? sender, System.EventArgs e)
    {
        ...
        _vm = vm;
        vm.Mecanicos.CollectionChanged += AoMudarColecao;
    ...
    private void RegerarTabela()
    {
        ...
        Control corpo = _vm.Mecanicos.Count > 0 ? CriarCorpo() : CriarMensagemVazia();
    ...
    private ScrollViewer CriarCorpo()
    {
        ...
        for (var i = 0; i < _vm!.Mecanicos.Count; i++)
            grid.RowDefinitions.Add(new RowDefinition(28, GridUnitType.Pixel));

        for (var i = 0; i < _vm.Mecanicos.Count; i++)
        {
            var mecanico = _vm.Mecanicos[i];
            ...
            {
                if (fundo.Tag is MecanicoDto dto && _vm.AbrirCommand.CanExecute(dto))
            ...
            Celula(grid, i, 0, mecanico.CodMecanico.ToString(), mono: true);
            Celula(grid, i, 1, mecanico.Nome);
            // Telefone: mono, célula vazia quando não informado (sem traço longo).
            Celula(grid, i, 2, mecanico.Telefone ?? string.Empty, mono: true);
            BadgeStatus(grid, i, 3, mecanico.FlgAtivo);
        }
    ...
    private void AoTeclarNaLista(object? sender, KeyEventArgs e)
    {
        ...
        switch (e.Key)
        {
            ...
            case Key.Up:
                ...
                break;
            case Key.Enter:
                if (_indiceSelecionado >= 0 && _linhas[_indiceSelecionado].Tag is MecanicoDto dto
    ...
    private static Border CriarMensagemVazia() => new()
    {
        ...
        Child = new TextBlock
        {
            Text = "Nenhum mecânico cadastrado. Clique em \"+ Novo mecânico\" para começar.",
            FontSize = 12,

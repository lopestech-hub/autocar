...
/// <summary>
/// Listagem de serviços (catálogo de mão de obra). Tabela renderizada como Grid único via
/// code-behind (padrão do projeto — nunca DataGrid). Duplo-clique numa linha abre o formulário.
/// </summary>
public partial class ServicosView : UserControl
{
    // CÓDIGO · DESCRIÇÃO · VALOR PADRÃO · STATUS
    private const string ColDefs = "80,*,120,90";
    private static readonly string[] Headers = { "CÓDIGO", "DESCRIÇÃO", "VALOR PADRÃO", "STATUS" };
    ...
    private void AoTrocarDataContext(object? sender, System.EventArgs e)
    {
        ...
        _vm = vm;
        vm.Servicos.CollectionChanged += AoMudarColecao;
    ...
    private void RegerarTabela()
    {
        ...
        Control corpo = _vm.Servicos.Count > 0 ? CriarCorpo() : CriarMensagemVazia();
    ...
    private ScrollViewer CriarCorpo()
    {
        ...
        for (var i = 0; i < _vm.Servicos.Count; i++)
        {
            ...
            {
                if (fundo.Tag is ServicoDto dto && _vm.AbrirCommand.CanExecute(dto))
            ...
            Celula(grid, i, 0, servico.CodServico.ToString(), mono: true);
            Celula(grid, i, 1, servico.Descricao);
            // Valor padrão: monetário (BR), alinhado à direita, fonte mono.
            Celula(grid, i, 2, servico.VlrPadrao.ToString("N2", new CultureInfo("pt-BR")), mono: true, direita: true);
            BadgeStatus(grid, i, 3, servico.FlgAtivo);
        }
    ...
    private void AoTeclarNaLista(object? sender, KeyEventArgs e)
    {
        ...
        switch (e.Key)
        {
            ...
            case Key.Up:
                ...
                break;
            case Key.Enter:
                if (_indiceSelecionado >= 0 && _linhas[_indiceSelecionado].Tag is ServicoDto dto
    ...
    private static Border CriarMensagemVazia() => new()
    {
        ...
        Child = new TextBlock
        {
            Text = "Nenhum serviço cadastrado. Clique em \"+ Novo serviço\" para começar.",
            FontSize = 12,

...
/// <summary>
/// Listagem de grupos de produto. Tabela renderizada por <see cref="ListBox"/> virtualizado.
...
{
    private GruposViewModel? _vm;

    public GruposView()
    {
        InitializeComponent();

        var lista = this.FindControl<ListBox>("ListaGrupos");
    ...
    {
        if (DataContext is not GruposViewModel vm)
            return;
    ...
    {
        if (sender is ListBox { SelectedItem: GrupoProdutoDto dto }

...
| **Decodificar imagens Cofap (`#C3p`)** | Julio, 2026-06-15: o acervo de fotos (≈6.581) vem do Catálogo Expresso Cofap em formato proprietário `#C3p` (ZIP cifrado por senha via `uz.dll`/Info-ZIP) — não é JPEG legível. A infra do ERP já está pronta (coluna `arquivo_imagem` + preview no Catálogo); falta gerar os JPGs reais. Plano de ataque e tudo investigado em **`E:\imagens_cofap\cofap-imagens-c3p.md`** (recomendação: interceptar a senha na chamada à `uz.dll` em runtime). É **projeto à parte**. | Quando houver tempo para o projeto de decodificação; o ERP exibe assim que os `.jpg` reais entrarem na pasta `Imagens:PastaBase` |
| **Popular e consumir a fonte/origem do dado de catálogo** | Estrutura de persistência **já criada** (2026-07-02, ver Histórico): tabela mestre `fonte_dado` + FK `produto.id_fonte`. Resta: (1) o `automacao_catalogo` **popular** `fonte_dado` e gravar `id_fonte` na carga (hoje a origem vive só no argumento `--marca` do script); (2) desenhar o **consumo** — exibição/filtro por origem no ERP e herança do campo na cópia para o AutoCar Expert (busca por IA). Ainda cobre `produto_similar` (que não tem `id_fonte`). | Quando a próxima carga do `automacao_catalogo` rodar (popular) e quando rastrear origem no ERP/Expert virar necessidade real |
...
| Data | Mudança |
| --- | --- |
| 2026-07-02 | **Rastreio de origem do dado (persistência)** (`CadastroFonteDado`): nova tabela mestre `fonte_dado` (id+cod, `descricao`, `sistema`, `observacao`, `flg_ativo`, `xmin` — molde Marca; **único composto `(descricao, sistema)`**) + coluna `produto.id_fonte` (FK **opcional**, `Restrict`). Migration aditiva. Registra de qual catálogo **e** por qual método a peça de referência foi extraída (ex: COFAP / `CATALOGO EXPRESSO (IDEIA2001)`), separado de `marca` (fabricante). Populado pelo `automacao_catalogo`; sem UI no ERP (só `Produto.DefinirFonte`). Detalhe em [Registrations](src/AutoCar.Desktop/Views/Registrations/MODULO.md) |
...
| 2026-06-15 | Feat (`b51a551`): **imagem do produto** — coluna `arquivo_imagem` (só o nome do arquivo; pasta-base `Imagens:PastaBase` por terminal), helper `ImagemProduto`, **preview no Catálogo via tooltip** (hover). Migration `RenomearArquivoImagem` (renomeia `url_imagem`→`arquivo_imagem` + limpa 366 caminhos legados). As imagens Cofap atuais são `#C3p` (não-JPEG) — infra pronta esperando JPGs reais; decodificação documentada em `E:\imagens_cofap` |
